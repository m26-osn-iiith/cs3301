...
1. Create a user program `user/test_sbrk` that calls `sbrk(1)` and `sbrklazy(1)`.
2. Add `$U/_test_sbrk` to `UPROGS` in Makefile.
3. Run your program in QEMU:
   ```bash
   make qemu
   ```
4. Inspect the process's page table before and after each call, and after accessing the lazily allocated memory.