...
Every process in xv6 has a parent (`p->parent`), and `fork()` is how it gets children. Add a new system call, `headcount()`, that returns the number of living children the calling process currently has: processes in the table whose `parent` field points to you, and whose state isn't `UNUSED` or `ZOMBIE`.
