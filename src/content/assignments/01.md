...
released: "2025-08-01"
due: "2025-09-11"
summary: "Build a C shell, a reliable UDP networking stack (S.H.A.M.), and extend xv6 with system calls and scheduling algorithms."
---
...
6. Ensure that the files are always listed in lexicographic order. Note that you are **not** required to implement the format of `ls -l`. (In fact if you do you may lose marks due to the automated evaluation!) Further note that here you must use the ASCII values of the characters to sort the names lexicographically.
7. If the directory does not exist, output "No such directory!"
8. If reveal is passed too many arguments, output "reveal: Invalid Syntax!"
9. If `reveal -` is run before any `hop` commands have been run since the start of the shell, output "No such directory!"
...
> **Note**: Implement this command after implementing Part C.1.
...
This part was implicitly required, and has just been added explicitly for clarity. You must allow the execution of **arbitrary commands**. This includes commands like `cat`, `echo`, `sleep`, etc. If a command does not exist (example `dosakdaoskdos`), output `Command not found!`.
...
4. After a user inputs a command, before parsing the input, the shell must check for completed background processes.
...
- **Sliding Window:** The sender can transmit multiple packets without waiting for an acknowledgment for each one. The number of unacknowledged packets in flight must not exceed a fixed window size (e.g., 10 packets).
...
**Assumptions:** Sliding Window = 4 packets, Packet Data Size = 1024 bytes, Sequence Numbers are byte-based.
...
The sender's timer for packet 2 (`SEQ=1025`) expires. It retransmits **only that packet**.
...
The receiver gets the retransmitted packet 2 and can now process its buffered packets (3 and 4).
...
**Answer:** No. The final `ACK=4097` is a cumulative acknowledgment confirming all data up to byte 4096 was received. The sender cancels timers for packets 3 and 4 and continues sending new data.
...
- The sender must ensure that the amount of unacknowledged data in flight (`LastByteSent - LastByteAcked`) is **always ≤** the receiver's advertised `window_size`.
...
**Client — File Transfer Mode (Default):**
...
- `--chat`: Activates Chat Mode. When used, all file-related arguments are ignored.
- `[loss_rate]`: Optional float between 0.0 and 1.0 indicating packet loss probability. Defaults to 0.0.

### 3.2 Mode-Specific Behavior \[10\]

- **File Transfer Mode (Default):** After the handshake, the client sends the specified file to the server. The server receives and saves the file.
- **Chat Mode (`--chat`):** After the handshake, both client and server enter a loop to handle concurrent input from `stdin` and the network socket. Typing `/quit` initiates the 4-way `FIN` handshake to terminate the connection.

> **Hint:** Use the `select()` system call to monitor both `stdin` (fd 0) and your socket to handle I/O without threads.

### 3.3 Standardized Output for Verification \[5\]

**This requirement applies to File Transfer Mode only.** Upon successfully receiving the entire file and closing the connection, the server **must**:

1. Calculate the MD5 checksum of the received file.
2. Print to `stdout` in the exact format: `MD5: <32-character_lowercase_md5_hash>`

**Linux — Install and compile:**
...
The receiver must programmatically drop incoming data packets based on the optional `loss_rate` argument to test retransmission logic. This applies to both modes.

## 4. Verbose Logging for Evaluation \[10\]

To allow the evaluator to verify the internal mechanics of your protocol, implement verbose logging that writes to a dedicated log file.

- **Activation:** Set environment variable `RUDP_LOG=1`. If not set, no log file is created.
- **Log Files:** `server_log.txt` for the server, `client_log.txt` for the client.
- **Log Line Format:** `[YYYY-MM-DD HH:MM:SS.microseconds] [LOG] event_description`

Use `gettimeofday()` from `<sys/time.h>` for microsecond-level timestamps:
...
The default scheduling policy in xv6 is round-robin. Your task is to implement two additional scheduling policies. The kernel should use only one scheduling policy declared at compile time, defaulting to round-robin if none is specified.

Modify the Makefile to support the `SCHEDULER` macro:
- First Come First Serve: `FCFS`
- Completely Fair Scheduler: `CFS`

### First Come First Serve \[20\]

Modify the xv6 scheduler from round-robin to FCFS. The scheduler must find the RUNNABLE process with the earliest creation time and select it for execution. Only when the currently RUNNING process terminates is another process selected. Comment out the original round-robin scheduler code before adding your FCFS version.

Compile with: `make clean; make qemu SCHEDULER=FCFS`

**Hints:**
- Use preprocessor directives to declare the alternate scheduling policy in `scheduler()` in `kernel/proc.c`.
- Edit `struct proc` in `kernel/proc.h` to add process creation time.
- Modify `allocproc()` to set up values when the process starts.

> **Note:** Use `procdump` (see `kernel/proc.c`) for debugging — it prints process state to the console when you type Ctrl-P.

### B.1: Priority Support \[10\]

1. Add a nice value for each process.
2. Calculate system weight based on nice value:
   - Nice 0: weight = 1024
...
1. Track the **virtual runtime** of each process — the amount of CPU time consumed, normalized by system weight.
...
1. Maintain processes in order of their `vruntime` (ascending).
...
Compile with: `make clean; make qemu SCHEDULER=CFS`

### B.4: Time Slice Calculation \[20\]

1. Define a target latency of 48 ticks.
2. Calculate time slice as: `time_slice = target_latency / number_of_runnable_processes`
3. Enforce a minimum time slice of 3 ticks.
4. Each process runs for its calculated time slice before being preempted.

## Report \[20\]

Add logging to print the vRuntime of all runnable processes before every scheduling decision:
...
- A brief explanation of your implementation for each spec.
- A performance comparison between Round Robin, FCFS, and CFS, showing average waiting and running times (use 1 CPU). Use the [schedulertest](https://iiithydresearch-my.sharepoint.com/:u:/g/personal/prakhar_jain_research_iiit_ac_in/ER9ZYNvTcdlLhtTf_jrs1sgBEmsKOfn_VImzk8dJk4eEdA?e=nbYHq6) command to collect this data.

# Bonus: Simplified Preemptive MLFQ Scheduler for xv6 \[25\]

> **Important:** This bonus counts towards the overall course bonus, not this assignment specifically.
...
- **Time Slice Expiry:** If a process uses its full time slice, move it to the end of the next lower queue (unless already in queue 3, keep it there).
...
- **Starvation Prevention:** Every 48 ticks, move all processes to queue 0.
- **Completed processes** should leave the system.
- If you attempt this section, add a MLFQ comparison to the report.

> **Notes:** "Tick" refers to the clock interrupt timer in `kernel/trap.c`. Preemption can only occur at tick boundaries.
