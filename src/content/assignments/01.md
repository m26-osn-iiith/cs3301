...
You are required to submit your submission via Moodle. Ensure that your submission contains the `.git` folder. Do not submit any build files (run `make clean` before zipping your folder).
...
[Code - Makefile](/code/mp1/Makefile)
...
7. Entries are always listed in lexicographic order, sorted by ASCII value. When the `-t` flag is passed, entries within each directory must also be listed in lexicographic order.
