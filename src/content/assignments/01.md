...
> **Note:** `cat` is `Stopped` because a background process group that tries to read the terminal is sent `SIGTTIN` (see D2 #12). `sort` is still `Running`, it is only blocked on the pipe.
